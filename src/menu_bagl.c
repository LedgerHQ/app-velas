...
const char *const pubkey_display_data_getter_values[] = {"Long", "Short",
                                                         "Back"};
...
void settings_submenu_selector(unsigned int idx) {
  switch (idx) {
  case 0:
    ux_menulist_init_select(0, allow_blind_sign_data_getter,
                            allow_blind_sign_data_selector,
                            N_storage.settings.allow_blind_sign);
    break;
  case 1:
    ux_menulist_init_select(0, pubkey_display_data_getter,
