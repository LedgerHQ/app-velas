  ...
  BEGIN_TRY {
    TRY {
      os_perso_derive_node_bip32_seed_key(HDW_ED25519_SLIP10, CX_CURVE_Ed25519,
                                          derivationPath, derivationPathLength,
                                          privateKeyData, NULL,
...
void handleSignMessage(uint8_t p1, uint8_t p2, uint8_t *dataBuffer,
  ...
  if ((p2 & P2_EXTEND) == 0) {
    ...
    G_derivationPathLength =
        read_derivation_path(dataBuffer, dataLength, G_derivationPath);
  ...
  if (process_message_body(parser.buffer, parser.buffer_length, &header)) {
    if (N_storage.settings.allow_blind_sign == BlindSignEnabled) {
      ...
      cx_hash_sha256(G_message, G_messageLength,
                     (uint8_t *)&UnrecognizedMessageHash, HASH_LENGTH);
