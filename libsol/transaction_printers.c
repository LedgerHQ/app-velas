...
#define is_create_stake_account(infos, infos_length)                           \
  instruction_infos_match_briefs(infos, create_stake_account_brief,            \
                                 infos_length)
...
#define is_create_stake_account_checked(infos, infos_length)                   \
  instruction_infos_match_briefs(infos, create_stake_account_checked_brief,    \
                                 infos_length)
...
#define is_create_stake_account_with_seed_checked(infos, infos_length)         \
  instruction_infos_match_briefs(                                              \
      infos, create_stake_account_with_seed_checked_brief, infos_length)
...
#define is_stake_split_with_seed_v1_1(infos, infos_length)                     \
  instruction_infos_match_briefs(infos, stake_split_with_seed_brief_v1_1,      \
                                 infos_length)
...
#define is_stake_authorize_both(infos, infos_length)                           \
  instruction_infos_match_briefs(infos, stake_authorize_both_brief,            \
                                 infos_length)
...
#define is_stake_authorize_checked_both(infos, infos_length)                   \
  instruction_infos_match_briefs(infos, stake_authorize_checked_both_brief,    \
                                 infos_length)
...
#define is_create_nonce_account(infos, infos_length)                           \
  instruction_infos_match_briefs(infos, create_nonce_account_brief,            \
                                 infos_length)
...
#define is_create_vote_account_with_seed(infos, infos_length)                  \
  instruction_infos_match_briefs(infos, create_vote_account_with_seed_brief,   \
                                 infos_length)
...
#define is_spl_token_create_mint(infos, infos_length)                          \
  instruction_infos_match_briefs(infos, spl_token_create_mint_brief,           \
                                 infos_length)
...
#define is_spl_token_create_mint_with_seed(infos, infos_length)                \
  instruction_infos_match_briefs(infos, spl_token_create_mint_with_seed_brief, \
                                 infos_length)
...
#define is_spl_token_create_account(infos, infos_length)                       \
  instruction_infos_match_briefs(infos, spl_token_create_account_brief,        \
                                 infos_length)
...
#define is_spl_token_create_account2(infos, infos_length)                      \
  instruction_infos_match_briefs(infos, spl_token_create_account2_brief,       \
                                 infos_length)
...
#define is_spl_token_create_account_with_seed(infos, infos_length)             \
  instruction_infos_match_briefs(                                              \
      infos, spl_token_create_account_with_seed_brief, infos_length)
...
#define is_spl_token_create_account2_with_seed(infos, infos_length)            \
  instruction_infos_match_briefs(                                              \
      infos, spl_token_create_account2_with_seed_brief, infos_length)
...
#define is_spl_token_create_multisig_with_seed(infos, infos_length)            \
  instruction_infos_match_briefs(                                              \
      infos, spl_token_create_multisig_with_seed_brief, infos_length)
...
#define is_spl_associated_token_account_create_with_transfer(infos,            \
                                                             infos_length)     \
  instruction_infos_match_briefs(                                              \
      infos, spl_associated_token_account_create_with_transfer_brief,          \
      infos_length)

static int print_create_stake_account(const MessageHeader *header,
...
static int print_create_stake_account_with_seed(const MessageHeader *header,
                                                InstructionInfo *const *infos,
                                                size_t infos_length) {
...
static int print_stake_split_with_seed(const MessageHeader *header,
                                       InstructionInfo *const *infos,
...
static int print_stake_authorize_both(const MessageHeader *header,
                                      InstructionInfo *const *infos,
                                      size_t infos_length) {
  ...
  if (staker_info->new_authority == withdrawer_info->new_authority) {
    item = transaction_summary_general_item();
    summary_item_set_pubkey(item, "New authorities",
    ...
    item = transaction_summary_general_item();
    summary_item_set_pubkey(item, "New withdraw auth",
...
static int print_create_nonce_account(const MessageHeader *header,
                                      InstructionInfo *const *infos,
                                      size_t infos_length) {
  const SystemCreateAccountInfo *ca_info = &infos[0]->system.create_account;
...
static int print_create_nonce_account_with_seed(const MessageHeader *header,
                                                InstructionInfo *const *infos,
                                                size_t infos_length) {
...
static int print_create_vote_account(const MessageHeader *header,
                                     InstructionInfo *const *infos,
                                     size_t infos_length) {
  const SystemCreateAccountInfo *ca_info = &infos[0]->system.create_account;
...
static int print_create_vote_account_with_seed(const MessageHeader *header,
                                               InstructionInfo *const *infos,
                                               size_t infos_length) {
  const SystemCreateAccountWithSeedInfo *ca_info =
      &infos[0]->system.create_account_with_seed;
...
static int print_vote_authorize_both(const MessageHeader *header,
                                     InstructionInfo *const *infos,
                                     size_t infos_length) {
...
static int print_spl_token_create_mint(const MessageHeader *header,
                                       InstructionInfo *const *infos,
                                       size_t infos_length) {
  const SystemCreateAccountInfo *ca_info = &infos[0]->system.create_account;
  ...
  if (im_info->freeze_authority != NULL) {
    item = transaction_summary_general_item();
    summary_item_set_pubkey(item, "Freeze authority",
                            im_info->freeze_authority);
  }
...
static int print_spl_token_create_account(const MessageHeader *header,
                                          InstructionInfo *const *infos,
                                          size_t infos_length) {
  const SystemCreateAccountInfo *ca_info = &infos[0]->system.create_account;
...
static int print_spl_token_create_multisig(const MessageHeader *header,
                                           InstructionInfo *const *infos,
                                           size_t infos_length) {
  const SystemCreateAccountInfo *ca_info = &infos[0]->system.create_account;
  ...
  item = transaction_summary_general_item();
  summary_item_set_multisig_m_of_n(item, im_info->body.m,
                                   im_info->signers.count);
...
static int print_spl_token_create_mint_with_seed(const MessageHeader *header,
                                                 InstructionInfo *const *infos,
                                                 size_t infos_length) {
  const SystemCreateAccountWithSeedInfo *ca_info =
      &infos[0]->system.create_account_with_seed;
...
static int
print_spl_token_create_account_with_seed(const MessageHeader *header,
                                         InstructionInfo *const *infos,
                                         size_t infos_length) {
  const SystemCreateAccountWithSeedInfo *ca_info =
      &infos[0]->system.create_account_with_seed;
...
static int
print_spl_token_create_multisig_with_seed(const MessageHeader *header,
                                          InstructionInfo *const *infos,
                                          size_t infos_length) {
  const SystemCreateAccountWithSeedInfo *ca_info =
      &infos[0]->system.create_account_with_seed;
...
static int print_spl_associated_token_account_create_with_transfer(
    const MessageHeader *header, InstructionInfo *const *infos,
    size_t infos_length) {
...
int print_transaction(const MessageHeader *header,
  ...
  case 1: {
    ...
    case ProgramIdSplAssociatedTokenAccount:
      return print_spl_associated_token_account_info(
          &display_info->spl_associated_token_account, header);
  ...
  case 2: {
    if (is_create_stake_account(infos, infos_length) ||
      ...
      return print_stake_split_with_seed(header, infos, infos_length, true);
      ...
      return print_stake_split_with_seed(header, infos, infos_length, false);
      ...
      return print_spl_token_create_account(header, infos, infos_length);
    } else if (is_spl_token_create_multisig(infos, infos_length)) {
      return print_spl_token_create_multisig(header, infos, infos_length);
      ...
      return print_spl_token_create_account_with_seed(header, infos,
                                                      infos_length);
    } else if (is_spl_token_create_multisig_with_seed(infos, infos_length)) {
      return print_spl_token_create_multisig_with_seed(header, infos,
                                                       infos_length);
