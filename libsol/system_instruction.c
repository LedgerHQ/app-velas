...
static int parse_system_instruction_kind(Parser *parser,
...
int parse_system_instructions(const Instruction *instruction,
                              const MessageHeader *header, SystemInfo *info) {
  ...
  switch (info->kind) {
  case SystemTransfer:
    return parse_system_transfer_instruction(&parser, instruction, header,
                                             &info->transfer);
  case SystemAdvanceNonceAccount:
    return parse_system_advance_nonce_account_instruction(
        &parser, instruction, header, &info->advance_nonce);
  case SystemCreateAccount:
    return parse_system_create_account_instruction(&parser, instruction, header,
                                                   &info->create_account);
  case SystemCreateAccountWithSeed:
    return parse_system_create_account_with_seed_instruction(
        &parser, instruction, header, &info->create_account_with_seed);
  case SystemInitializeNonceAccount:
    return parse_system_initialize_nonce_account_instruction(
        &parser, instruction, header, &info->initialize_nonce);
  case SystemWithdrawNonceAccount:
    return parse_system_withdraw_nonce_account_instruction(
        &parser, instruction, header, &info->withdraw_nonce);
  case SystemAuthorizeNonceAccount:
    return parse_system_authorize_nonce_account_instruction(
        &parser, instruction, header, &info->authorize_nonce);
  case SystemAssign:
    return parse_system_assign_instruction(&parser, instruction, header,
                                           &info->assign);
  case SystemAllocate:
    return parse_system_allocate_instruction(&parser, instruction, header,
                                             &info->allocate);
  case SystemAllocateWithSeed:
    return parse_system_allocate_with_seed_instruction(
        &parser, instruction, header, &info->allocate_with_seed);
  case SystemAssignWithSeed:
    break;
  }
...
static int
print_system_advance_nonce_account(const SystemAdvanceNonceInfo *info,
                                   const MessageHeader *header) {
  SummaryItem *item;
...
static int
print_system_authorize_nonce_info(const SystemAuthorizeNonceInfo *info,
                                  const MessageHeader *header) {
  SummaryItem *item;
...
int print_system_info(const SystemInfo *info, const MessageHeader *header) {
  ...
  case SystemCreateAccount:
    return print_system_create_account_info(CREATE_ACCOUNT_TITLE,
                                            &info->create_account, header);
  case SystemCreateAccountWithSeed:
    return print_system_create_account_with_seed_info(
        CREATE_ACCOUNT_TITLE, &info->create_account_with_seed, header);
  case SystemInitializeNonceAccount:
    return print_system_initialize_nonce_info("Init nonce acct",
                                              &info->initialize_nonce, header);
  case SystemWithdrawNonceAccount:
  ...
  case SystemAllocateWithSeed:
    return print_system_allocate_with_seed_info(
        "Allocate acct", &info->allocate_with_seed, header);
  case SystemAssignWithSeed:
    break;
  }
