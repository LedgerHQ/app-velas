...
void test_summary_item_setters() {
  ...
  assert(strncmp("test", item.sized_string.string, item.sized_string.length) ==
         0);
...
void test_transaction_summary_update_display_for_item() {
  ...
  item.kind = SummaryItemNone;
  assert(transaction_summary_update_display_for_item(&item, DisplayFlagNone) ==
         1);

  summary_item_set_amount(&item, "amount", 42);
  assert(transaction_summary_update_display_for_item(&item, DisplayFlagNone) ==
         0);
  assert_transaction_summary_display("amount", "0.000000042 VLX");
  ...
  assert(transaction_summary_update_display_for_item(
             &item, DisplayFlagLongPubkeys) == 0);
  assert_transaction_summary_display("pubkey",
  ...
  assert(transaction_summary_update_display_for_item(&item, DisplayFlagNone) ==
         0);
  assert_transaction_summary_display("hash",
                                     "11111111111111111111111111111111");
...
#define display_item_test_helper(item, item_index)                             \
  do {                                                                         \
    ...
    assert(transaction_summary_display_item(item_index, DisplayFlagNone) ==    \
           0);                                                                 \
...
#define display_item_test_helper_general_item(general_index)                   \
  do {                                                                         \
    ...
    assert(transaction_summary_display_item(general_index + 1,                 \
                                            DisplayFlagNone) == 0);            \
...
void test_repro_unrecognized_format_reverse_nav_hash_corruption_bug() {
  ...
  Pubkey fee_payer = {{0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
  ...
  Hash message_hash = {{0x0f, 0x1e, 0x6b, 0x14, 0x21, 0xc0, 0x4a, 0x07,
