...
static int parse_vote_instruction_kind(Parser *parser,
...
int parse_vote_instructions(const Instruction *instruction,
                            const MessageHeader *header, VoteInfo *info) {
  ...
  switch (info->kind) {
  case VoteInitialize:
    return parse_vote_initialize_instruction(&parser, instruction, header,
                                             &info->initialize);
  case VoteWithdraw:
    return parse_vote_withdraw_instruction(&parser, instruction, header,
                                           &info->withdraw);
  case VoteAuthorize:
    return parse_vote_authorize_instruction(&parser, instruction, header,
                                            &info->authorize);
  case VoteAuthorizeChecked:
    return parse_vote_authorize_checked_instruction(&parser, instruction,
                                                    header, &info->authorize);
  case VoteUpdateValidatorId:
    return parse_vote_update_validator_id_instruction(
        &parser, instruction, header, &info->update_validator_id);
  case VoteUpdateCommission:
    return parse_vote_update_commission_instruction(
        &parser, instruction, header, &info->update_commission);
  case VoteVote:
  case VoteSwitchVote:
    break;
  }
...
static int print_vote_withdraw_info(const VoteWithdrawInfo *info
                                    // const MessageHeader* header
) {
  SummaryItem *item;
...
static int
print_vote_update_validator_id_info(const VoteUpdateValidatorIdInfo *info
                                    // const MessageHeader* header
) {
  SummaryItem *item;
...
static int
print_vote_update_commission_info(const VoteUpdateCommissionInfo *info
                                  // const MessageHeader* header
) {
  SummaryItem *item;
...
int print_vote_info(const VoteInfo *info) {
  switch (info->kind) {
  case VoteInitialize:
    return print_vote_initialize_info("Init vote acct", &info->initialize
                                      // header
    );
  case VoteWithdraw:
    return print_vote_withdraw_info(&info->withdraw
                                    // header
    );
  case VoteAuthorize:
  case VoteAuthorizeChecked:
    return print_vote_authorize_info(&info->authorize
                                     // header
    );
  case VoteUpdateValidatorId:
    return print_vote_update_validator_id_info(&info->update_validator_id
                                               // header
    );
  case VoteUpdateCommission:
    return print_vote_update_commission_info(&info->update_commission
                                             // header
    );
  case VoteVote:
  case VoteSwitchVote:
...
int print_vote_initialize_info(const char *primary_title,
  ...
  item = transaction_summary_general_item();
  summary_item_set_pubkey(item, "New vote auth",
