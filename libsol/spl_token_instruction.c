...
static int parse_spl_token_instruction_kind(Parser *parser,
...
static int parse_initialize_account_spl_token_instruction(
    Parser *parser, const Instruction *instruction, const MessageHeader *header,
    SplTokenInitializeAccountInfo *info, bool expect_owner_in_accounts) {
  InstructionAccountsIterator it;
  instruction_accounts_iterator_init(&it, header, instruction);
...
static int parse_spl_token_sign(InstructionAccountsIterator *it,
                                SplTokenSign *sign) {
  size_t n = instruction_accounts_iterator_remaining(it);
  BAIL_IF(n == 0);
...
static int parse_token_authority_type(Parser *parser,
...
static const char *
stringify_token_authority_type(Token_AuthorityType auth_type) {
  switch (auth_type) {
  case Token_AuthorityType_MintTokens:
    return "Mint tokens";
  case Token_AuthorityType_FreezeAccount:
...
static int
parse_close_account_spl_token_instruction(const Instruction *instruction,
                                          const MessageHeader *header,
...
static int
parse_freeze_account_spl_token_instruction(const Instruction *instruction,
                                           const MessageHeader *header,
...
static int
parse_thaw_account_spl_token_instruction(const Instruction *instruction,
                                         const MessageHeader *header,
...
static int
parse_sync_native_spl_token_instruction(const Instruction *instruction,
                                        const MessageHeader *header,
...
int parse_spl_token_instructions(const Instruction *instruction,
  ...
  case SplTokenKind(InitializeMint):
    return parse_initialize_mint_spl_token_instruction(
        &parser, instruction, header, &info->initialize_mint);
  case SplTokenKind(InitializeAccount):
    return parse_initialize_account_spl_token_instruction(
        &parser, instruction, header, &info->initialize_account, true);
  case SplTokenKind(InitializeAccount2):
    return parse_initialize_account_spl_token_instruction(
        &parser, instruction, header, &info->initialize_account, false);
  case SplTokenKind(InitializeMultisig):
    return parse_initialize_multisig_spl_token_instruction(
        &parser, instruction, header, &info->initialize_multisig);
  case SplTokenKind(Revoke):
    return parse_revoke_spl_token_instruction(instruction, header,
                                              &info->revoke);
  case SplTokenKind(SetAuthority):
    return parse_set_authority_spl_token_instruction(&parser, instruction,
                                                     header, &info->set_owner);
  case SplTokenKind(CloseAccount):
    return parse_close_account_spl_token_instruction(instruction, header,
                                                     &info->close_account);
  case SplTokenKind(FreezeAccount):
    return parse_freeze_account_spl_token_instruction(instruction, header,
                                                      &info->freeze_account);
  case SplTokenKind(ThawAccount):
    return parse_thaw_account_spl_token_instruction(instruction, header,
                                                    &info->thaw_account);
  case SplTokenKind(TransferChecked):
    return parse_transfer_spl_token_instruction(&parser, instruction, header,
                                                &info->transfer);
  case SplTokenKind(ApproveChecked):
    return parse_approve_spl_token_instruction(&parser, instruction, header,
                                               &info->approve);
  case SplTokenKind(MintToChecked):
    return parse_mint_to_spl_token_instruction(&parser, instruction, header,
                                               &info->mint_to);
  case SplTokenKind(BurnChecked):
    return parse_burn_spl_token_instruction(&parser, instruction, header,
                                            &info->burn);
  case SplTokenKind(SyncNative):
    return parse_sync_native_spl_token_instruction(instruction, header,
                                                   &info->sync_native);
  // Deprecated instructions
  case SplTokenKind(Transfer):
  case SplTokenKind(Approve):
...
static int
print_spl_token_initialize_mint_info(const char *primary_title,
...
static int print_spl_token_initialize_account_info(
    const char *primary_title, const SplTokenInitializeAccountInfo *info,
    const MessageHeader *header) {
  SummaryItem *item;
...
static int print_spl_token_initialize_multisig_info(
    const char *primary_title, const SplTokenInitializeMultisigInfo *info,
    const MessageHeader *header) {
  SummaryItem *item;
...
int print_spl_token_transfer_info(const SplTokenTransferInfo *info,
  ...
  summary_item_set_token_amount(item, "Transfer tokens", info->body.amount,
                                symbol, info->body.decimals);
...
static int print_spl_token_approve_info(const SplTokenApproveInfo *info,
  ...
  item = transaction_summary_general_item();
  const char *symbol = get_token_symbol(info->mint_account);
  summary_item_set_token_amount(item, "Allowance", info->body.amount, symbol,
                                info->body.decimals);
...
print_spl_token_set_authority_info(const SplTokenSetAuthorityInfo *info,
  ...
  const char *authority_type =
      stringify_token_authority_type(info->authority_type);
...
static int
print_spl_token_close_account_info(const SplTokenCloseAccountInfo *info,
                                   const MessageHeader *header) {
  SummaryItem *item;
...
static int
print_spl_token_thaw_account_info(const SplTokenThawAccountInfo *info,
                                  const MessageHeader *header) {
  SummaryItem *item;
...
int print_spl_token_info(const SplTokenInfo *info,
                         const MessageHeader *header) {
  switch (info->kind) {
  case SplTokenKind(InitializeMint):
    return print_spl_token_initialize_mint_info("Init mint",
                                                &info->initialize_mint, header);
  case SplTokenKind(InitializeAccount):
  case SplTokenKind(InitializeAccount2):
    return print_spl_token_initialize_account_info(
        "Init acct", &info->initialize_account, header);
  case SplTokenKind(InitializeMultisig):
    return print_spl_token_initialize_multisig_info(
        "Init multisig", &info->initialize_multisig, header);
  case SplTokenKind(Revoke):
...
const Pubkey *
spl_token_option_pubkey_get(const SplTokenOptionPubkey *option_pubkey) {
