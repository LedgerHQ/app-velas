...
void test_parse_spl_token_create_token() {
  ...
  assert(parse_instruction(&parser, &instruction) ==
         0); // SystemCreateAccount (ignored)
  assert(instruction_validate(&instruction, &header) == 0);
  assert(parse_instruction(&parser, &instruction) ==
         0); // SplTokenInitializeMint
  assert(instruction_validate(&instruction, &header) == 0);
...
void test_parse_spl_token_create_account2() {
  uint8_t message[] = {
      0x02, 0x00, 0x04, 0x06, OWNER_ACCOUNT, TOKEN_ACCOUNT, MINT_ACCOUNT,
      SYSVAR_RENT, PROGRAM_ID_SYSTEM, PROGRAM_ID_SPL_TOKEN, BLOCKHASH, 0x02,
      // SystemCreateAccount
      0x04, 0x02, 0x00, 0x01, 0x34, 0x00, 0x00, 0x00, 0x00, 0x80, 0x56, 0x1a,
      0x00, 0x00, 0x00, 0x00, 0x00, 0x78, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00, PROGRAM_ID_SPL_TOKEN,
      // SplTokenInitializeAccount2
      0x05, 0x03, 0x01, 0x02, 0x03, 0x21, 0x10, OWNER_ACCOUNT};
...
void test_parse_spl_token_create_multisig() {
  uint8_t message[] = {2, 0, 5, 8, OWNER_ACCOUNT, MULTISIG_ACCOUNT, SYSVAR_RENT,
                       SIGNER1, SIGNER2, SIGNER3, PROGRAM_ID_SYSTEM,
                       PROGRAM_ID_SPL_TOKEN, BLOCKHASH, 2,
                       // SystemCreateAccount
                       6, 2, 0, 1, 52, 0, 0, 0, 0, 245, 1, 0, 0, 0, 0, 0, 0, 40,
                       0, 0, 0, 0, 0, 0, 0, PROGRAM_ID_SPL_TOKEN,
...
void test_print_m_of_n_string() {
  ...
  assert(print_m_of_n_string(Token_MAX_SIGNERS, Token_MAX_SIGNERS, s,
                             sizeof(s)) == 0);
