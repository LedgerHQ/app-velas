...
int process_message_body(const uint8_t *message_body, int message_body_length,
  ...
  for (; instruction_count < header->instructions_length; instruction_count++) {
    ...
    case ProgramIdSplAssociatedTokenAccount: {
      if (parse_spl_associated_token_account_instructions(
              &instruction, header, &info->spl_associated_token_account) == 0) {
        info->kind = program_id;
      }
      break;
    }
    ...
    case ProgramIdSplToken:
      if (parse_spl_token_instructions(&instruction, header,
                                       &info->spl_token) == 0) {
        info->kind = program_id;
      }
      break;
  ...
  return print_transaction(header, display_instruction_info,
