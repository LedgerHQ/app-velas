...
static int parse_stake_instruction_kind(Parser *parser,
...
static int parse_stake_authorize(Parser *parser,
...
static int parse_stake_lockupargs(Parser *parser, StakeLockup *lockup,
...
int parse_stake_instructions(const Instruction *instruction,
                             const MessageHeader *header, StakeInfo *info) {
  ...
  switch (info->kind) {
  case StakeDelegate:
    return parse_delegate_stake_instruction(instruction, header,
                                            &info->delegate_stake);
  case StakeInitialize:
    return parse_stake_initialize_instruction(&parser, instruction, header,
                                              &info->initialize);
  case StakeInitializeChecked:
    return parse_stake_initialize_checked_instruction(
        &parser, instruction, header, &info->initialize);
  case StakeWithdraw:
    return parse_stake_withdraw_instruction(&parser, instruction, header,
                                            &info->withdraw);
  case StakeAuthorize:
    return parse_stake_authorize_instruction(&parser, instruction, header,
                                             &info->authorize);
  case StakeAuthorizeChecked:
    return parse_stake_authorize_checked_instruction(&parser, instruction,
                                                     header, &info->authorize);
  case StakeDeactivate:
    return parse_stake_deactivate_instruction(&parser, instruction, header,
                                              &info->deactivate);
  case StakeSetLockup:
    return parse_stake_set_lockup_instruction(&parser, instruction, header,
                                              &info->set_lockup);
  case StakeSetLockupChecked:
    return parse_stake_set_lockup_checked_instruction(
        &parser, instruction, header, &info->set_lockup);
  case StakeSplit:
    return parse_stake_split_instruction(&parser, instruction, header,
                                         &info->split);
  case StakeMerge:
...
static int print_delegate_stake_info(const StakeDelegateInfo *info,
                                     const MessageHeader *header) {
  SummaryItem *item;
...
int print_stake_info(const StakeInfo *info, const MessageHeader *header) {
  ...
  case StakeInitialize:
  case StakeInitializeChecked:
    return print_stake_initialize_info("Init stake acct", &info->initialize,
                                       header);
  case StakeWithdraw:
...
int print_stake_split_info1(const StakeSplitInfo *info,
                            const MessageHeader *header) {
  SummaryItem *item;
