...
void summary_item_set_amount(SummaryItem *item, const char *title,
                             uint64_t value) {
...
void summary_item_set_token_amount(SummaryItem *item, const char *title,
...
void summary_item_set_u64(SummaryItem *item, const char *title,
                          uint64_t value) {
...
void summary_item_set_pubkey(SummaryItem *item, const char *title,
                             const Pubkey *value) {
...
void summary_item_set_hash(SummaryItem *item, const char *title,
                           const Hash *value) {
...
void summary_item_set_sized_string(SummaryItem *item, const char *title,
...
void summary_item_set_string(SummaryItem *item, const char *title,
                             const char *value) {
...
void summary_item_set_timestamp(SummaryItem *item, const char *title,
                                int64_t value) {
...
static int
transaction_summary_update_display_for_item(const SummaryItem *item,
                                            enum DisplayFlags flags) {
  switch (item->kind) {
  case SummaryItemNone:
    return 1;
  case SummaryItemAmount:
    BAIL_IF(print_amount(item->u64, G_transaction_summary_text,
  ...
  case SummaryItemTokenAmount:
    BAIL_IF(print_token_amount(item->token_amount.value,
                               ...
                               G_transaction_summary_text, TEXT_BUFFER_LENGTH));
    break;
  case SummaryItemI64:
    BAIL_IF(
        print_i64(item->i64, G_transaction_summary_text, TEXT_BUFFER_LENGTH));
    break;
  case SummaryItemU64:
    BAIL_IF(
        print_u64(item->u64, G_transaction_summary_text, TEXT_BUFFER_LENGTH));
    break;
  case SummaryItemPubkey: {
    ...
    if (flags & DisplayFlagLongPubkeys) {
      BAIL_IF(print_string(tmp_buf, G_transaction_summary_text,
                           TEXT_BUFFER_LENGTH));
    } else {
      BAIL_IF(print_summary(tmp_buf, G_transaction_summary_text,
                            BASE58_PUBKEY_SHORT, SUMMARY_LENGTH,
                            SUMMARY_LENGTH));
    }
    break;
  }
  case SummaryItemHash:
    BAIL_IF(encode_base58(item->hash, BLOCKHASH_SIZE,
                          G_transaction_summary_text, TEXT_BUFFER_LENGTH));
    break;
...
int transaction_summary_finalize(enum SummaryItemKind *item_kinds,
