...
#define VLX_DECIMALS 9
int print_amount(uint64_t amount, char *out, size_t out_length) {
  return print_token_amount(amount, "VLX", VLX_DECIMALS, out, out_length);
}

int print_sized_string(const SizedString *string, char *out,
                       size_t out_length) {
...
int print_summary(const char *in, char *out, size_t out_length,
