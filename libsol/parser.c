...
int parse_pubkeys(Parser *parser, PubkeysHeader *header,
...
static int parse_data(Parser *parser, const uint8_t **data,
                      size_t *data_length) {
...
int parse_instruction(Parser *parser, Instruction *instruction) {
  ...
  BAIL_IF(parse_data(parser, &instruction->accounts,
