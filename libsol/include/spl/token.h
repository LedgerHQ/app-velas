...
typedef enum Token_TokenInstruction_Tag {
   ...
   * will be equal to the amount of VLX in the account. If this account is
   ...
   * amounts of VLX and Tokens will be transferred to the destination
   * account.
   *
   * Accounts expected by this instruction:
   *
  ...
  /**
   * Close an account by transferring all its VLX to the destination account.
  ...
  /**
   * Given a wrapped / native token account (a token account containing VLX)
   * updates its amount field based on the account's underlying `lamports`.
   * This is useful if a non-wrapped VLX account uses
...
typedef struct Token_Mint {
   ...
   * provided during mint creation. If no mint authority is present then the
   * mint has a fixed supply and no further tokens may be minted.
   */
...
typedef struct Token_Account {
   ...
   * reserve. An Account is required to be rent-exempt, so the value is used by
   * the Processor to ensure that wrapped VLX accounts do not drop below this
   * threshold.
   */
