...
enum ProgramId instruction_program_id(const Instruction *instruction,
  ...
  } else if (memcmp(program_id, &spl_associated_token_account_program_id,
                    PUBKEY_SIZE) == 0) {
    return ProgramIdSplAssociatedTokenAccount;
  } else if (memcmp(program_id, &serum_assert_owner_program_id, PUBKEY_SIZE) ==
             0) {
...
int instruction_validate(const Instruction *instruction,
                         const MessageHeader *header) {
  BAIL_IF(instruction->program_id_index >=
...
bool instruction_info_matches_brief(const InstructionInfo *info,
...
size_t
instruction_accounts_iterator_remaining(const InstructionAccountsIterator *it) {
  if (it->current_instruction_account < it->instruction_accounts_length) {
