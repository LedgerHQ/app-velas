...
static int parse_create_spl_associated_token_account_instruction(
    const Instruction *instruction, const MessageHeader *header,
...
int parse_spl_associated_token_account_instructions(
  ...
  return parse_create_spl_associated_token_account_instruction(
      instruction, header, &info->create);
}
...
int print_spl_associated_token_account_info(
    const SplAssociatedTokenAccountInfo *info, const MessageHeader *header) {
  return print_spl_associated_token_account_create_info(&info->create, header);
}
